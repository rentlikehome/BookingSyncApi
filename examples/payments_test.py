...
for page in range(50, pages + 1):
    ...
    for payment in data["payments"]:
        parsedStartDate = datetime.datetime.strptime(
            payment["bookings"][0]["start_at"], "%Y-%m-%dT%H:%M:%SZ"
        ).date()
        if parsedStartDate <= datetime.date(
            2021, 7, 31
        ) and parsedStartDate >= datetime.date(2021, 7, 1):
            row = []
