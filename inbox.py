...
def export_messages(filename, updated_since):
    ...
    if updated_since:
        base_messages_url += (
            f"&updated_since={updated_since.strftime(BOOKINGSYNC_DATE_FORMAT)}"
        )
        ...
        for message in messages:
            ...
            try:
                ...
                if message["sender"]["links"]["member"]["type"] == "Host":
                    ...
                    row.append(
                        f"{host['firstname']} {host['lastname']} ({host['email']})"
                    )
                else:
                    row.append("-")
...
def export_conversations():
    ...
    for page in range(1, pages + 1):
        conversations = api.get(f"/inbox/conversations?page={page}").json()[
            "conversations"
        ]
